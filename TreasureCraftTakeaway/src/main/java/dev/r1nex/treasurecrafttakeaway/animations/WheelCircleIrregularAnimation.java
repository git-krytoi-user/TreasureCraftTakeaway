...
public class WheelCircleIrregularAnimation {
    ...
    public WheelCircleIrregularAnimation(TreasureCraftTakeaway plugin, YamlConfiguration yamlConfiguration, double radiusRotation,
        ...
        async(() -> new BukkitRunnable() {
            ...
            public void run() {
                ...
                for (int entityIds : plugin.getEntityIds()) {
                    double x = radiusRotation * Math.sin(angle) * Math.cos(2 * angle);
                    double y = -radiusRotation * Math.sin(angle) * Math.pow(-1, (int) (angle/Math.PI));
    ...
    public void async(Runnable runnable) {
        Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
            synchronized (WheelCircleIrregularAnimation.this) {
                runnable.run();
            }
        });
    }
