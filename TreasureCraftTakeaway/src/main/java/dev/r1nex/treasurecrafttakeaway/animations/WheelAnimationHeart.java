...
public class WheelAnimationHeart {
    ...
    public WheelAnimationHeart(TreasureCraftTakeaway plugin, YamlConfiguration yamlConfiguration, double radiusRotation,
        ...
        location.setY(location.getY() + 0.350);
        location.setZ(location.getZ() + 0.01);
    }
        ...
        async(() -> new BukkitRunnable() {
            ...
            public void run() {
                ...
                for (int entityIds : plugin.getEntityIds()) {
                    double temp = Math.abs(Math.sin(angle));
                    double x = radiusRotation * Math.sin(angle) * Math.sqrt(temp);
                    double y = -radiusRotation * (Math.cos(angle) + 1) * Math.sqrt(temp);
                ...
                if (yamlConfiguration.getBoolean("settings.push-back")) {
                    for (Player bukkitPlayers : Bukkit.getOnlinePlayers()) {
                        boolean isNearCircle = isPlayerNearCircle(bukkitPlayers, loc, Math.PI / offset);
                        if (isNearCircle) {
                            bounceBack(bukkitPlayers, loc);
                        }
                    }
                }
            }
    ...
    public void async(Runnable runnable) {
        Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
            synchronized (WheelAnimationHeart.this) {
                runnable.run();
            }
        });
    }
        ...
        if (ticks >= 160) {
            ...
            plugin.getServer().getScheduler().runTaskTimer(plugin, (task) -> {
                if (queue.size() <= 1) {
                    ...
                    radiusRotation -= 0.0001 * 2 * Math.PI;
                    speedRotation -= 0.0001 * 2 * Math.PI;
                    return;
                }
    ...
    public boolean isPlayerNearCircle(Player player, Location circleCenter, double circleRadius) {
        Location playerLocation = player.getLocation();
        double distance = circleCenter.distance(playerLocation);
